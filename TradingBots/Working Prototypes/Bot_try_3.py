...
local_peak = -1
bot_balance = 0.00300692   #[In asset format, ex. (BTC in BTCUSDT)]                                        #initial_bot_ballance to allocate funds [Pending]
profit = 0 #fake
   ...
   #                 timeInForce=TIME_IN_FORCE_GTC,
    ...
    if (trading_state == SIDE_BUY):                                                                         
        db_dict['buy_in_price'].append(current_pr)                                                          #append Buy Details to db
                                                                                                            #[Pending, add buy-in amount(in_BTC or in_USDT)]
    else:
        db_dict['sell_out_price'].append(current_pr)                                                        #append Sell Details to db
        db_dict['profit_value'].append((db_dict['sell_out_price'][-1] - db_dict['buy_in_price'][-1]))       #Sell_Price, Profit_Value(in USD), Profit_Percentage(per Trade) [fake value]
        db_dict['profit_pct'].append(db_dict['profit_value'][-1]*100/db_dict['buy_in_price'][-1])           #[fake values][change values after implimenting balance system][Pending]
                                                                                                            #impliment balance system [Pending]
